...
SpellCheck.checkService = function(mistakes, fn) {
  ...
  for(var i = 0; i < (mistakes || []).length; i++) {
    ...
    // the segment used
    var segment = mistakes[i].token || mistakes[i].word;
  ...
  async.eachLimit(lowWords, 10, function(word, cb) {
    ...
    request({
      ...
      // set the response
      serverCachedResponses[segment] = '' + body;
    ...
    for(var i = 0; i < mistakes.length; i++) {
      ...
      // get the segment
      var segment = mistakes[i].token || mistakes[i].word || null;

      // must be defined
      if(!segment) continue;

      console.log(segment + ': ' + serverCachedResponses[(segment || '').toLowerCase()]);
