...
SpellCheck.checkService = function(mistakes, fn) {
  ...
  // check each
  async.eachLimit(_.uniq(mistakes), 10, function(mistake, cb) {

    // the word to check
    var word = mistake.word;

    // check if accronym
    if((word || '').toUpperCase() == word) return cb(null);

    // check if already defined ?
    if(serverCachedResponses[word]) {

      // add if bad
      if(serverCachedResponses[word] === 'BAD') {
    ...
    // check if already defined ?
    if(serverCachedResponses[word.token]) {

      // add if bad
      if(serverCachedResponses[word.token] === 'BAD') {
    ...
    request({
      ...
      form:     {
        ...
        word:     S(word || '').chompLeft('.').chompRight('.').trim().s.toLowerCase(),
      ...
      // set the response
      serverCachedResponses[mistake.word] = body;

      // must be a 200
      if(body === 'BAD' && !mistake.token) {

        // add the word with context information
        finalMistakes.push(mistake);
        ...
        request({
          ...
          form:     {
            ...
            word:     S(mistake.token || '').chompLeft('.').chompRight('.').trim().s.toLowerCase(),
            token:    settings.SPELLCHECK_API_KEY
          ...
          // set the response
          serverCachedResponses[mistake.token] = body;
...
SpellCheck.getLanguageCodes = function(fn) {
  ...
  SpellCheck.getLanguages(function(err, languages) {
    ...
    // define globally to speed up lokups
...
SpellCheck.getLanguage = function(code, fn) {
  ...
  SpellCheck.getLanguages(function(err, languages) {
    ...
    // clean the code
    var localCode = code.replace(/\-/gi, '_');
...
SpellCheck.check = function(params, fn) {
  ...
  for(var i = 0; i < content.length; i++) {
    ...
    cleanedWord = S(cleanedWord).chompLeft('.').chompRight('.').trim().s
