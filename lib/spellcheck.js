...
SpellCheck.checkService = function(mistakes, fn) {
  ...
  if(!process.env.PASSMARKED_INTERNAL_SPELLCHECK_API)
  ...
  async.eachLimit(mistakes, 10, function(mistake, cb) {
    ...
    request({
      ...
      url:      process.env.PASSMARKED_INTERNAL_SPELLCHECK_API,
      timeout:  1000 * 5,
      method:   'POST',
      type:     'POST',
      form:     {

        word:     (mistake || '').toLowerCase(),
        token:    process.env.PASSMARKED_INTERNAL_SPELLCHECK_KEY
