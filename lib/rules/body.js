  ...
  payload.getPageContent(function(err, content) {
    ...
    for(var i = 0; i < elementTags.length; i++) {
      ...
      $('body ' + elementTags[i]).each(function(index, elem) {
        ...
        // local ref
        var segment = spellCheck.trim( $(elem).contents().filter(function() {
        ...
        // check if no a acronym
        if((segment || '').toUpperCase() == segment) return;

        // check if not empty
        if(S(segment).isEmpty() === true) return;

        // push the item
        spellingTexts.push(segment);
    ...
    spellCheck.check({
        ...
        content:    spellingTexts.slice(0, 1000),
        ...
        for(var i = 0; i < mistakes.length; i++) {
          ...
          var build = payload.getSnippetManager().build(
            ...
            function(line) {
              ...
              var ww = mistake.word.toLowerCase();
              return ll.indexOf('<' + elementTags[i]) != -1 && 
                      (ll.indexOf(' ' + ww) != -1 || 
                        ll.indexOf(ww + ' ') != -1);
          ...
          // build the message
          var instance_message = '$ was not recognized';
          var identifiers_strs = [ mistake.word ];

          // check if the token is defined
          if(mistake.token) {

            instance_message = '$ in $ was not recognized';
